...
void PS2_Set_Int(u8 en)
{
	...
	if(en)EXTI->IMR|=1<<2;//不屏蔽line2上的中断
    else EXTI->IMR&=~(1<<2);//屏蔽line2上的中断   
}
//等待PS2时钟线sta状态改变
//sta:1，等待变为1;0，等待变为0;
...
u8 Wait_PS2_Scl(u8 sta)
{
	...
	while(PS2_SCL==sta)
	{
		Delay(1);
		t++;
		if(t>2600)return 1;//时间溢出 (设备会在10ms内检测这个状态)
	}
	return 0;//被拉低了
}
...
u8 PS2_Get_Byte(void)
{
	...
	while(1)//最大等待55ms
	{
		...
		if(PS2_Status&0x80)//得到了一次数据
		{
			...
			break;	
		}else if(t>850||PS2_Status&0x40)break;//超时溢出/接收错误	   
	}
...
void PS2_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_1 | GPIO_Pin_2; // PS2_SCL  PC.3 PS2输入 
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU ;  //上拉输入
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
	GPIO_Init(GPIOC, &GPIO_InitStructure);		  	  	 
 
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOC, GPIO_PinSource2);  	//选择PA1所在的GPIO管脚用作外部中断线路EXIT1   		   
	 			
 NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);	//设置NVIC中断分组2:2位抢占优先级，2位响应优先级
	EXTI_InitStructure.EXTI_Line = EXTI_Line2;	//外部线路EXIT15
